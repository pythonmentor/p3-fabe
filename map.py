...
class Map:
    ...
    def pos_items(self):
        ...
        self.items_poss = [] # Create an empty list
        for x, col in enumerate(self.structure):
            for y, case in enumerate(col):
                if case == "0":
                    position_items = (x, y)
                    self.items_poss.append(position_items) # Create a list of possible positions
        self.pos_items = random.sample(self.items_poss, 3) # Choose 3 random positions in the list

    def display(self, hero_x, hero_y): # Display the maze and its elements
        ...
        for x, col in enumerate(self.structure):
            for y, case in enumerate(col):
                ...
                pos_y = y * SPRITE_SIZE
                if case == "m":
                    self.window.blit(wall_img, (pos_x, pos_y)) # Display a wall image on the maze
                elif case == "d":
                    self.window.blit(macgyver_img, (hero_x, hero_y)) # Display Macgyver image on the maze
                elif case == "a":
                    self.window.blit(guardian_img, (pos_x, pos_y)) # Display the guardian image on the maze
    ...
    def display_win(self): # Display the victory image
        self.window.fill((0, 0, 0))
        win_img = pygame.image.load('ressource/win.jpg').convert_alpha()
        self.window.blit(win_img, (0, 0))
        pygame.display.flip()

    def display_lose(self): # Display the defeat image
        self.window.fill((0, 0, 0))
